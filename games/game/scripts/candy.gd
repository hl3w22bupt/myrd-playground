...
## 所有形体外接半径 ≤ RADIUS = 38；棋盘格 CELL = 96，半格 48 ——
## 余量 = 48 − 38 = 10px，保证相邻糖果之间至少 20px 缝隙，光标框（半边 44）始终框得住。
...
## 糖果半径（棋盘格 CELL=96，糖果外接半径 38，留 10px/边 缝隙，见文件头余量推导）。
const RADIUS: float = 38.0
## 圆角方形（橙子糖）的半边长（对角 √2×31 ≈ 44 仍小于 48 半格，包络不越界）。
const SQUARE_HALF: float = 31.0
## 高光通用色。
const GLOSS: Color = Color(1.0, 1.0, 1.0, 0.85)
const GLOSS_SOFT: Color = Color(1.0, 1.0, 1.0, 0.35)
## 环境光从左上来：底部暗弧的偏移量。
const SHADE_OFFSET: Vector2 = Vector2(0.0, 6.0)
...
func _draw_jelly(base: Color, dark: Color) -> void:
	...
	draw_circle(Vector2(0.0, -3.0), RADIUS - 4.0, base)
	# 底部反光：暗色圆盖在主体下半部，再叠一层更小的主体色，做出「果冻厚度」。
	draw_circle(SHADE_OFFSET, RADIUS - 4.0, Color(dark, 0.35))
	draw_circle(Vector2(0.0, -4.0), RADIUS - 8.0, base)
	draw_circle(Vector2(-12.0, -14.0), 9.0, GLOSS)
	draw_circle(Vector2(-16.0, -10.0), 4.0, Color(1.0, 1.0, 1.0, 0.95))
	draw_circle(Vector2(12.0, 10.0), 4.5, GLOSS_SOFT)
...
func _draw_lozenge(base: Color, dark: Color) -> void:
	...
	box.draw(get_canvas_item(), Rect2(-half, -half + 5.0, half * 2.0, half * 2.0))
	# 顶部亮边：一条圆角高光横带，模拟糖纸反光。
	var shine := StyleBoxFlat.new()
	shine.bg_color = Color(1.0, 1.0, 1.0, 0.28)
	shine.set_corner_radius_all(7)
	shine.draw(get_canvas_item(), Rect2(-half + 7.0, -half + 7.0, half * 2.0 - 14.0, 10.0))
	draw_circle(Vector2(-half + 14.0, half - 15.0), 5.0, GLOSS_SOFT)
...
func _draw_striped_ball(base: Color, dark: Color) -> void:
	...
	# 弦带：dy 处的半弦长 = sqrt(r² − dy²)，条带高度 8，覆盖球体中部。
	var stripe_half_height: float = 4.0
	for dy in [-15.0, 0.0, 15.0]:
		var half_width: float = sqrt(maxf((RADIUS - 5.0) * (RADIUS - 5.0) - dy * dy, 0.0))
		draw_rect(Rect2(-half_width + 1.0, dy - stripe_half_height - 2.0,
			(half_width - 1.0) * 2.0, stripe_half_height * 2.0),
			Color(1.0, 1.0, 1.0, 0.55))
	draw_circle(Vector2(-12.0, -15.0), 7.5, GLOSS)
	draw_circle(Vector2(13.0, 11.0), 4.0, GLOSS_SOFT)
...
func _draw_hexagon(base: Color, dark: Color) -> void:
	...
	for i in 6:
		...
		inner_points.append(Vector2.from_angle(angle_in) * (RADIUS - 8.0))
	draw_colored_polygon(inner_points, base)
	# 顶部高光带：上三条边内侧的一条圆角亮带。
	var shine := StyleBoxFlat.new()
	shine.bg_color = Color(1.0, 1.0, 1.0, 0.30)
	shine.set_corner_radius_all(6)
	shine.draw(get_canvas_item(), Rect2(-15.0, -25.0, 30.0, 11.0))
	draw_circle(Vector2(10.0, 8.0), 4.5, GLOSS_SOFT)
