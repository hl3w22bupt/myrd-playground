...
#   2 = 环境不可用（找不到 Godot / python3 / 技能包脚本，装环境而不是改代码）
...
# 帧预算 240：本工程冒烟要跑「移动/游走 + 收集 + 边界钳制 + 四次穿越危机 + 冻结 +
# 重开 + 两次换幕 + 独活/耗竭/真心三分支」约 170 个物理帧，默认 120 的兜底余量太薄 ——
# headless 下 process:physics 不严格相等，协程没跑完就被兜底杀掉会「既无 PASS 也无 FAIL」。
# 预算只放宽兜底，不改变判定语义。
