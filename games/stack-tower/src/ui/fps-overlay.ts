...
export function frameStats(durationsMs: number[]): FrameStats {
  const n = durationsMs.length;
  if (n === 0) return { count: 0, p95Ms: 0, jankCount: 0, avgFps: 0 };
...
export function withinFrameBudget(stats: FrameStats): boolean {
  return stats.p95Ms <= 18.2 && stats.jankCount === 0;
}

export interface FpsOverlayHandle {
  /** 每帧喂一次帧耗时（ms）；窗口满后滚动淘汰 */
  sample(durationMs: number): FrameStats;
  stats(): FrameStats;
  mount(host: HTMLElement | null): void;
  dispose(): void;
}

export function createFpsOverlay(): FpsOverlayHandle {
  const window: number[] = [];
  let el: HTMLElement | null = null;
  let latest: FrameStats = frameStats([]);
  return {
    sample(durationMs) {
