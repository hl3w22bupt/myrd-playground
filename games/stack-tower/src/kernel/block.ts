...
import type { MovingBlock } from './types.js';

/** 生成新摆动块：宽度继承顶部已落块；自中轴（塔顶 x）入画；初始方向由 RNG 决定。
 *  入画相位取中轴（offset=0）：保证开局第 1 次输入即存在可命中窗口（T1 必改①，QNC-02 无开局死局）。 */
export function spawnMoving(rng: Rng, level: number, centerX: number, width: number): MovingBlock {
...
export function advanceMoving(m: MovingBlock, centerX: number, dtMs: number): void {
