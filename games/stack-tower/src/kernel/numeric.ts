...
export function perfectWindowMs(level: number): number {
  const { PERFECT_WINDOW_BASE_MS, PERFECT_WINDOW_DECAY_MS_PER_LEVEL, PERFECT_WINDOW_MIN_MS } =
    NUMERIC.perfect_window;
...
export function swingSpeed(level: number): number {
...
/** 本关层目标：8 + (level−1)×2 */
export function targetLayers(level: number): number {
...
export function perfectDistance(level: number): number {
  return (swingSpeed(level) * perfectWindowMs(level)) / 1000;
}

/** tower-ripple 表现时长：名义 300ms，合法区间 [250,350]（spec.content.towerRipple） */
export const RIPPLE_DURATION = { NOMINAL_MS: 300, TOLERANCE_MS: 50, MIN_MS: 250, MAX_MS: 350 } as const;
