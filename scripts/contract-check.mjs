...
function collectNumericSources() {
  ...
  for (const entity of spec.entities ?? []) {
    ...
    const hit = resolveAgainstProject(entity.script);
    if (hit && hit.endsWith(".gd")) sources.set(fs.realpathSync(hit), fs.readFileSync(hit, "utf8"));
  }
  const fallbackList = [
  ...
  const EXCLUDE = new Set([".git", ".godot", "node_modules", "export", "qa", ".myrd", ".myrd-platform"]);
  const found = [];
  const walk = (dir, depth) => {
    ...
    for (const entry of entries) {
      ...
      else if (entry.name.endsWith(".gd")) found.push(full);
    }
  };
  walk(projectDir, 0);
